...
exports.createOrder = async (req, res, next) => {
  try {
    const { feeId, amount, currency = 'INR', receipt, notes } = req.body;
    ...
    if (!feeId && amount && amount > 0) {
      ...
      // Save to PaymentOrder without feeId but with studentId if available
      await PaymentOrder.create({ 
        orderId: order.id,
        studentId: notes?.studentId || null, // Store student ID from notes
        amount: order.amount, 
    ...
    const order = await client.orders.create({ 
      amount: Math.round(amount * 100), 
      currency, 
      receipt: receipt || `FEE_${feeId}_${Date.now()}`,
      notes: notes || {}
    });
